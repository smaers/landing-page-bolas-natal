const CONFIG = {
  ...
  shippingApiUrl: '/api/freight',
  trackingApiUrl: '/api/tracking',
  // Preencha no ambiente do backend/serverless. Não coloque token privado aqui.
  originCep: 'SEU_CEP_DE_ORIGEM'
};
...
function buildOrderText(){
  ...
  if(shippingQuote) lines.push(`Frete: ${shippingQuote.free ? 'GRÁTIS' : brl(shippingQuote.price)}${shippingQuote.service ? ' — '+shippingQuote.service : ''}`);
...
$('quoteShipping').addEventListener('click', async ()=>{
  ...
  try{
    ...
    try{
      const api=await fetch(CONFIG.shippingApiUrl,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({recipientCep:cep,recipientCity:via.localidade,recipientState:via.uf,shipmentInvoiceValue:total,originCep:CONFIG.originCep})});
      ...
      shippingQuote={free:false,price:Number(options[0].price||0),city:via.localidade,service:options[0].service||'Transportadora'};
      result.innerHTML=`<div class="result-icon">🚚</div><h3>Frete calculado</h3><p>${via.localidade} - ${via.uf}</p><div class="freight-option"><span>${shippingQuote.service}</span><strong>${brl(shippingQuote.price)}</strong></div>`;
      updateSummary();
    }catch(err){
      shippingQuote=null;
...
$('trackButton').onclick=async()=>{
  ...
  try{
    ...
    out.innerHTML = `O código foi recebido. Para consultar o status em tempo real, configure o endpoint <code>/api/tracking</code> com sua conta de transporte.`;
  }
};
